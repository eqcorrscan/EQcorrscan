...
# We are going to look for detections within one day of data on the 4th of
# Jan 2016, which is the same day as our templates
...
# Setting this to be higher than the number of templates will have no increase
# in speed as only detections for each template are computed in parallel.
ncores = 4
...
detections = match_filter.match_filter(template_names=template_names,
                                       ...
                                       cores=ncores, tempdir=False, debug=0,
                                       plot_format='jpg')

# Now lets try and work out how many unique events we have just to compare with
# the GeoNet catalog of 20 events on this day in this sequence
unique_detections = []
...
for detection in unique_detections:
    ...
    stplot.trim(starttime=detection.detect_time - 10,
                endtime=detection.detect_time + 30)
