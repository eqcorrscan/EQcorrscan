...
def reverse_template_gen(templates, streams, FAR, save=False, cores=1, debug=0):
    """
    ...
    :type templates: list
    :param templates: list of templates of which each template is a \
		    stream of obspy traces
    :type streams: list
    :param streams: a list of daylong obspy streams for correlation.  The length\	of the list should be the number of days to take FAR number of false\
		    detections over.
    :type FAR: int
    :param FAR: number of false detections allowable for the duration of streams.
    :type save: bool
    ...
    for i, st in enumerate(streams):
	    reversed_cccsums, reversed_nochans, reversed_chans = _channel_loop(reversed_templates, st, cores=cores, debug=debug)
	    if i == 0:
    ...
    if save:
	    headerstr = 'templates: '
	    for template in templates:
		    headerstr = headerstr+template[0].stats.starttime.strftime('%Y%m%dT%H%M%S')+';'
	    np.savetxt('cccsums_reversed_template.txt', rev_cccsum, fmt='%10.5f', delimiter=',', newline='\n', header=headerstr)
    thresholds = []
    maxccs = []
    meanccs = []
    for rsum in rev_cccsum:
	    ...
	    threshold = heapq.nlargest(FAR, abs(rsum))
...
def match_filter(template_names, template_list, st, threshold,
    ...
    if arg_check:
        ...
        if str(threshold_type) not in [str('MAD'), str('absolute'),
                                       str('av_chan_corr'), str('reverse_temp')]:
            msg = 'threshold_type must be one of: MAD, absolute, av_chan_corr, reverse_temp'
            raise MatchFilterError(msg)
        for tr in st:
