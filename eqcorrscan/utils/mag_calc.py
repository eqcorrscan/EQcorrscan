...
if __name__ == '__main__':
    """
    Code to loop through a database and make a lot of magnitude picks.

    Coded to run through SEISAN databases.
    """
    import sys
    if len(sys.argv) < 6 or len(sys.argv) > 7:
        msg = 'Insufficient arguments, needs the database to calculate ' +\
            'over and the ouptut database, paths to REA dir (not year/mm ' +\
            'dirs) for both please, and the path to the CAL directory, and ' +\
            'the start and stop dates as yyyymmddhhmmss'
        raise IOError(msg)
    ...
    import datetime as dt
    try:
        startdate = dt.datetime.strptime(startdate.ljust(14, '0'),
                                         '%Y%m%d%H%M%S')
    except:
        raise IOError('start date is not yyyymmddhhmmss form')
    try:
        stopdate = dt.datetime.strptime(enddate.ljust(14, '0'),
                                        '%Y%m%d%H%M%S')
    except:
