...
int multi_normalise_2(float *ccc, int ccc_len, float *image, float norm_sum, int template_len, int n)
{
	int i, j, k;
	...
	for(i=1; i<ccc_len; ++i)
	{
		...
		else
		{
		    ...
		    for (k=0; k<n; ++k)
		    {
		        ccc[(i * n) + k] = (ccc[(i * n) + k] - norm_sum * mean ) / std;
		    }
    	}
	}
	return 0;
