...
if len(sys.argv) == 2:
    ...
    if flag == '--debug':
        ...
        raise IOError("I don't recognise your arguments I know --debug and " +
                      "--debug-prep, and --instance, --splits, --startdate, " +
                      "--enddate")
elif len(sys.argv) == 5:
    ...
    if args[0] == '--instance' or args[2] == '--instance':
        ...
        for i in xrange(len(args)):
            if args[i] == '--startdate':
                ...
                print 'Start date is: '+dt.datetime.strftime(startdate,
                                                             '%Y/%m/%d')
            elif args[i] == '--enddate':
                enddate = dt.datetime.strptime(str(args[i+1]), '%Y/%m/%d')
                print 'End date is: '+dt.datetime.strftime(enddate, '%Y/%m/%d')
    else:
        raise IOError("I don't recognise your arguments,  I know --debug " +
                      "and --debug-prep, and --instance, --splits, " +
                      "--startdate, --enddate")
elif len(sys.argv) == 7:
    args = sys.argv[1:len(sys.argv)]
    Split = False
    ...
    raise ValueError("I only take one argument, no arguments, or two flags " +
                     "with arguments")
else:
    Test = False
    Prep = False
...
from core import birght_lights, match_filter
...
template_names = glob.glob('templates/Picked_stacks/*_template.ms')
...
for template in templates:
    ...
    for tr in template:
        if not templatedef.samp_rate*templatedef.length == tr.stats.npts:
            raise ValueError('Template for ' + tr.stats.station + '.' +
                             tr.stats.channel + ' is not the correct ' +
                             'length, recut. It is: ' + str(tr.stats.npts) +
                             ' and should be ' +
    ...
    for tr in template:
        ...
        if len(tr.stats.channel) == 3:
            ...
            raise ValueError('Channels are not named with either three or ' +
                             'two characters')
stations = list(set(stations))
# Use the templates to scan through the datas!
# Work out what days are to be scanned through
if startdate:
    dates = [UTCDateTime(startdate)+i
             for i in xrange(0, int(UTCDateTime(enddate) -
                                    UTCDateTime(startdate)), 86400)]
else:
...
f = open('detections/picked_stacks_'+str(min(dates).year) +
...
for day in dates:
    ...
    for stachan in stations:
        ...
        rawdir = '/projects/nesi00219/logfiles/Volumes/Taranaki_01/' +\
            'data/boseca/SAMBA_mar09/'+station+'/' +\
        ...
        if not Test:
            ...
            if chan_available:
                ...
                # Add to this list only if we have the data
            else:
    ...
    # Before we merge, we should check that we have long enough data,
    # the GeoNet data are often
    # missing sections for some reason...?
    st = st.merge(fill_value='interpolate')  # Enforce trace continuity
    st_check = st.copy()
    st_check = st_check.merge()  # Merge without filling
    for tr in st_check:
        tr_check = tr.copy()
        tr_check = tr_check.split()
    trace_length = 0
    for section in tr_check:
        trace_length += len(section.data)
    if trace_length < section.stats.sampling_rate * 72000:
        # Require at least 20 hours of data
        print('Removing data for ' + tr.stats.station +
              ' because it has less than 20 hours of data')
        st.remove(tr)
    del tr_check, st_check
    if 'st' not in locals():
        print 'No data found for day: '+str(day)
    elif len(actual_stations) < matchdef.minsta:
        print 'Data from fewer than '+str(matchdef.minsta) +\
            ' stations found, will not detect'
    else:
        if not Test:
            # Process data
        ...
        if not Prep:
            ...
            if 'stream_st' in locals():
                del stream_st
            for tr in st:
                if 'stream_st' not in locals():
                    stream_st = Stream(tr)
                else:
                    stream_st += tr
            st = stream_st
            # Call the match_filter module - returns detections, a list of
            # detections containted within the detection class with elements,
            # time, template, number of channels used and cross-channel
            # correlation sum.
            ...
            max_templates_int = 16
            if len(all_templates) > max_templates_int:
                groups = int(len(all_templates) / max_templates_int)
            for i in xrange(groups):
                print 'Running group number ' + str(i)
                if i == groups:
                    templates = all_templates[i * max_templates_int:]
                    template_names = all_template_names[i * max_templates_int:]
                else:
                    templates = all_templates[i * max_templates_int:(i + 1) *
                                              max_templates_int]
                    template_names = all_template_names[i * max_templates_int:
                                                        (i + 1) *
                                                        max_templates_int]
                print 'I am running this group with '+str(len(templates)) +\
                    ' templates'
                ...
                for detection in detections:
                    ...
                    f.write(detection.template_name+', ' +
                            str(detection.detect_time) +
                            ', '+str(detection.detect_val) +
                            ', '+str(detection.threshold) +
                            ', '+str(detection.no_chans)+'\n')
                    print 'template: '+detection.template_name +\
                        ' detection at: '+str(detection.detect_time) +\
                        ' with a cccsum of: '+str(detection.detect_val)
                if detections:
                    f.write('\n')
            del detections
        else:
