...
def _template_loop(template, chan, station, channel, i=0, output=False):
    """
    ...
    run.
    :type output: multiprocessing.Queue
    :param output: Set to False if not run from a multiprocessing.Process run.
    ...
    with Timer() as t:
        ...
        try:
            ...
            raise IndexError('No station, channel match for '+station+' '+channel)
...
def _channel_loop(templates, stream):
    ...
    from par import match_filter_par as matchdef
    from core import match_filter_internal
    from utils.timer import Timer
    parallel=True # Set to false for debugging, in general should be set to True
    ...
    for tr in stream:
        ...
        with Timer() as t:
            ...
            if parallel:
                results=[pool.apply_async(match_filter_internal._template_loop,\
                                          args=(templates[i].select(station=station,\
                                                                    channel=channel)[0].data,\
                                                            tr_data,\
                                                (templates[i].select(station=station, \
                                                                    channel=channel)[0].stats.starttime-\
                                                templates[i].sort(['starttime'])[0].stats.starttime)*\
                                                templates[i].select(station=station,\
                                                                    channel=channel)[0].stats.sampling_rate, i))\
                                      for i in xrange(len(templates))]
                # results=mp.Queue()
                # processes=[mp.Process(target=_template_loop, args=(templates[i],\
                                                            # tr_data, station,\
                                                                    # channel, i, results))\
                                      # for i in xrange(len(templates))]
                # for p in processes:
                ...
                cccs_list=[match_filter_internal._template_loop(templates[i].select(station=station,\
                                                                                    channel=channel)[0].data,\
                                                                tr_data,\
                                                                (templates[i].select(station=station, \
                                                                    channel=channel)[0].stats.starttime-\
        ...
        if matchdef.debug >=1:
            ...
            if not parallel:
                print " I have "+str(len(cccs_list))+" results"
        if parallel:
            with Timer() as t:
