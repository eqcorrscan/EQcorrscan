...
def moveout_check(template, nodes, lags, threshold, thresh_type, lowcut,\
    ...
    for i in xrange(len(nodes)):
        print "Progress: "+str((i/len(nodes))*100)
        for j in xrange(len(ccc)):
            pad=np.array([0]*int(round(lags[j,i]*image[0].stats.sampling_rate)))
        ...
        peaks = findpeaks.find_peaks2(cccsum, rawthresh,\
                                      image[0].stats.sampling_rate, 0)
        if not len(peaks) == 0:
            possible_locations+=[nodes[i]]
...
if __name__ == '__main__':
    ...
    from core.bright_lights import _read_tt
    template=read(templatename)
    stations=[]
    for tr in template:
        stations+=[tr.stats.station]
    stations=list(set(stations))
    stations, nodes, lags = _read_tt(brightdef.nllpath,stations,\
                                    brightdef.phase, phaseout='S', \
                                    ps_ratio=brightdef.ps_ratio)
    print np.shape(lags)
    for station in stations:
        if 'template_dummy' in locals():
